...
struct Dictionary * ParserLoad(const char * path)
{
    ...
    if (!IOHelperOpen(&in, path, "r"))
	{
        MessageError("ParserLoad", "Error when the application tried to load %s", path);
        return NULL;
    }
    ...
    while ( fgets(line+last, ASCIILINESZ-last, in) != NULL )
	{
        ...
        if (line[len] != '\n')
		{
			MessageError("ParserLoad", "Input line too long in %s (line: %d)", path, lineno);
            DictionaryDel(dict);
            fclose(in);
            return NULL;
        }
        ...
        switch (ParserLine(line, section, key, val))
		{
            ...
            case LINE_ERROR:
				MessageError("ParserLoad", "Syntax error in %s (line: %d):\n(debug: %s)", path, lineno, line);
				errs++;
				break;
        ...
        if (errs < 0)
		{
            MessageError("ParserLoad", "Memory allocation failure");
            break;
        }
    }
